...
run=wandb.init(
    ...
    project="WavLM_Large_10thLayer", # ===================================== Change this ==========================

    # track hyperparameters and run metadata
    config={
    "architecture": "simpleMLP",
    "dataset": "wavLM_Large_10thLayer",
    },reinit=True
)

# Path to feature files
path_features = 'WavLM/Wavlm_Large/wavlmLarge_InterFeat/10th_Layer/' #====================================== Change this ========================== 
extension     = 'csv'
...
# Process label files
gtzan_path_features = "WavLM/Wavlm_Large/wavlmLarge_InterFeat/10th_Layer/" #======================================= Change this ==========================
genre_label_map = {
    "blues": 0,
    "classical": 1,
    "country": 2,
...
best_model_path = 'WavLM/Wavlm_Large/train_10th_layer/best123-_model_10thLayerLarge.pth'  # ============================= Change this ===============
...
best_validation_loss = float('inf')
early_stop_counter = 0
best_model_path = 'WavLM/Wavlm_Large/train_10th_layer/best231-_model_10thLayerLarge.pth'  # ============================= Change this ===============
def calculate_accuracy(outputs, labels):
    _, predicted = torch.max(outputs.data, 1)
    total = labels.size(0)
    correct = (predicted == labels).sum().item()
...
best_validation_loss = float('inf')
early_stop_counter = 0
best_model_path = 'WavLM/Wavlm_Large/train_10th_layer/best312-_model_10thLayerLarge.pth'  # ============================= Change this ===============
def calculate_accuracy(outputs, labels):
    _, predicted = torch.max(outputs.data, 1)
    total = labels.size(0)
    correct = (predicted == labels).sum().item()
