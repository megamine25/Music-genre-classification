...
for file in file_list:
    ...
    # 20 layer ------------------------
    rep = layer_reps[19] # 20 layer
    rep[0].shape
    rep_np = rep[0].detach().cpu().numpy()
    rep_df = pd.DataFrame(rep_np)
    file_id = 'WavLM/Wavlm_Large/wavlmLarge_InterFeat/20th_Layer/'+file.split('/')[2].split('.')[0]+'.'+file.split('/')[2].split('.')[1]+'.csv'
    rep_df.to_csv(file_id)
    print(file_id) 

    # 21 layer ------------------------
    rep = layer_reps[20] # 21 layer
    rep[0].shape
    rep_np = rep[0].detach().cpu().numpy()
    rep_df = pd.DataFrame(rep_np)
    file_id = 'WavLM/Wavlm_Large/wavlmLarge_InterFeat/21th_Layer/'+file.split('/')[2].split('.')[0]+'.'+file.split('/')[2].split('.')[1]+'.csv'
    rep_df.to_csv(file_id)
    print(file_id)  

    # 22 layer ------------------------
    rep = layer_reps[21] # 22 layer
    rep[0].shape
    rep_np = rep[0].detach().cpu().numpy()
    rep_df = pd.DataFrame(rep_np)
    file_id = 'WavLM/Wavlm_Large/wavlmLarge_InterFeat/22th_Layer/'+file.split('/')[2].split('.')[0]+'.'+file.split('/')[2].split('.')[1]+'.csv'
    rep_df.to_csv(file_id)
    print(file_id)

    # 23 layer ------------------------
    rep = layer_reps[22] # 23 layer
    rep[0].shape
    rep_np = rep[0].detach().cpu().numpy()
    rep_df = pd.DataFrame(rep_np)
    file_id = 'WavLM/Wavlm_Large/wavlmLarge_InterFeat/23th_Layer/'+file.split('/')[2].split('.')[0]+'.'+file.split('/')[2].split('.')[1]+'.csv'
    rep_df.to_csv(file_id)
    print(file_id)
