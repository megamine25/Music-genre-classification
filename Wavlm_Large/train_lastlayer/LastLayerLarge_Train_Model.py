...
run=wandb.init(
    ...
    project="WavLM_Large_LastLayer_123", # ===================================== Change this ==========================

    # track hyperparameters and run metadata
    config={
    "architecture": "simpleMLP",
    "dataset": "wavLM_Large_LastLayer",
    },reinit=True
)

# Path to feature files
path_features = 'Wavlm_Large/wavlmlargefeat/' #======================================= Change this ==========================  'Wavlm_Base/wavlmBasefeat/'
extension     = 'csv'
...
# Process label files
gtzan_path_features = "Wavlm_Large/wavlmlargefeat/" #======================================= Change this ==========================
genre_label_map = {
    "blues": 0,
    "classical": 1,
    "country": 2,
...
best_model_path = 'Wavlm_Large/train_lastlayer/best123_model_LastLayerLarge.pth'  # ============================= Change this ===============
...
for epoch in range(num_epochs):
    ...
    for batch_X, batch_y in train_loader:
        batch_X, batch_y = batch_X.to("cuda:0"), batch_y.to("cuda:0", dtype=torch.int64)
        optimizer.zero_grad()
...
# Training set
with torch.no_grad():
    test_outputs = best_model(X_train.to("cuda:0"))
    test_loss    = criterion(test_outputs, y_train.to(torch.int64).to("cuda:0"))
...
# Validation set
with torch.no_grad():
    test_outputs = best_model(X_test.to("cuda:0"))
    test_loss    = criterion(test_outputs, y_test.to(torch.int64).to("cuda:0"))
...
# Test set
with torch.no_grad():
    test_outputs = best_model(features_tensor3.to("cuda:0"))
    test_loss    = criterion(test_outputs, labels_tensor3.to(torch.int64).to("cuda:0"))
...
best_validation_loss = float('inf')
early_stop_counter = 0
best_model_path = 'Wavlm_Large/train_lastlayer/best231_model_LastLayerLarge.pth'  # ============================= Change this ===============
def calculate_accuracy(outputs, labels):
    _, predicted = torch.max(outputs.data, 1)
    total = labels.size(0)
    correct = (predicted == labels).sum().item()
...
best_validation_loss = float('inf')
early_stop_counter = 0
best_model_path = 'Wavlm_Large/train_lastlayer/best312_model_LastLayerLarge.pth'  # ============================= Change this ===============
def calculate_accuracy(outputs, labels):
    _, predicted = torch.max(outputs.data, 1)
    total = labels.size(0)
    correct = (predicted == labels).sum().item()
