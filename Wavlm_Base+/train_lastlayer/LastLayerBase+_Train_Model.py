...
run=wandb.init(
    ...
    project="WavLM_Base+_LastLayer", # ===================================== Change this ==========================

    # track hyperparameters and run metadata
    config={
    "architecture": "simpleMLP",
    "dataset": "wavLM_Base+_LastLayer",
    },name="123 LastLayer",
    reinit=True
)

# Path to feature files
path_features = 'Wavlm_Base+/wavlmBase+feat/' #====================================== Change this ========================== 
extension     = 'csv'
...
# Process label files
gtzan_path_features = "Wavlm_Base+/wavlmBase+feat/" #======================================= Change this ==========================
genre_label_map = {
    "blues": 0,
    "classical": 1,
    "country": 2,
...
best_model_path = 'Wavlm_Base+/train_lastlayer/best123_model_LastLayerBase+.pth'  # ============================= Change this ===============
...
run=wandb.init(
    # set the wandb project where this run will be logged
    project="WavLM_Base+LastLayer", # ===================================== Change this ==========================
...
best_validation_loss = float('inf')
early_stop_counter = 0
best_model_path = 'Wavlm_Base+/train_lastlayer/best231_model_LastLayerBase+.pth'  # ============================= Change this ===============
def calculate_accuracy(outputs, labels):
    _, predicted = torch.max(outputs.data, 1)
    total = labels.size(0)
    correct = (predicted == labels).sum().item()
...
best_validation_loss = float('inf')
early_stop_counter = 0
best_model_path = 'Wavlm_Base+/train_lastlayer/best312_model_LastLayerBase+.pth'  # ============================= Change this ===============
def calculate_accuracy(outputs, labels):
    _, predicted = torch.max(outputs.data, 1)
    total = labels.size(0)
    correct = (predicted == labels).sum().item()
