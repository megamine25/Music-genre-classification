...
for file in file_list:
    ...
    file_id = 'WavLM/wavlmBase+_InterFeat/3rd_Layer/'+file.split('/')[2].split('.')[0]+'.'+file.split('/')[2].split('.')[1]+'.csv'
    ...
    rep_np = rep[0].detach().cpu().numpy()
    rep_df = pd.DataFrame(rep_np)
    file_id = 'WavLM/wavlmBase+_InterFeat/6th_Layer/'+file.split('/')[2].split('.')[0]+'.'+file.split('/')[2].split('.')[1]+'.csv'
    rep_df.to_csv(file_id)
    print(file_id)  
    ...
    rep_np = rep[0].detach().cpu().numpy()
    rep_df = pd.DataFrame(rep_np)
    file_id = 'WavLM/wavlmBase+_InterFeat/9th_Layer/'+file.split('/')[2].split('.')[0]+'.'+file.split('/')[2].split('.')[1]+'.csv'
    rep_df.to_csv(file_id)
    print(file_id)  
