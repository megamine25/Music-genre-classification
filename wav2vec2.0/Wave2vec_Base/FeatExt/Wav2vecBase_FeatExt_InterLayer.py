...
for file in file_list:
    ...
    rep6 = hidden_states[2]  # 3rd layer
    rep6 = rep6.squeeze().cpu().numpy()
    rep_df = pd.DataFrame(rep6)
    file_id = 'Wav2vec2/Wave2vec_Base/wav2vecbase_InterFeat/3rd_layer/'+file.split('/')[2].split('.')[0]+'.'+file.split('/')[2].split('.')[1]+'.csv'
    rep_df.to_csv(file_id)
    print(file_id)
#---------------------------------------
    rep12 = hidden_states[5]  # 6th layer
    rep12 = rep12.squeeze().cpu().numpy()
    rep_df = pd.DataFrame(rep12)
    file_id = 'Wav2vec2/Wave2vec_Base/wav2vecbase_InterFeat/6th_layer/'+file.split('/')[2].split('.')[0]+'.'+file.split('/')[2].split('.')[1]+'.csv'
    rep_df.to_csv(file_id)
    print(file_id)
#---------------------------------------
    rep18 = hidden_states[8]  # 9th layer
    rep18 = rep18.squeeze().cpu().numpy()
    rep_df = pd.DataFrame(rep18)
    file_id = 'Wav2vec2/Wave2vec_Base/wav2vecbase_InterFeat/9th_layer/'+file.split('/')[2].split('.')[0]+'.'+file.split('/')[2].split('.')[1]+'.csv'
    rep_df.to_csv(file_id)
    #print(file_id)
